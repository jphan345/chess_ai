...
class Board:
    ...
    def make_move(self, move):
        ...
        self.game_state_history.append(self.game_state.copy())

        move_color = Color.WHITE if self.game_state.white_to_move else Color.BLACK
        ...
        captured_piece = Piece.PAWN | self.game_state.opponent_color if is_en_passant else self.board[target_square]
        ...
        self.game_state.zobrist_key ^= self.zobrist.get_zobrist_num(moved_piece, start_square)
        self.game_state.zobrist_key ^= self.zobrist.get_zobrist_num(moved_piece, target_square)

        # reset en passant files and checks
        self.game_state.zobrist_key ^= self.zobrist.get_zobrist_en_passant(self.game_state.en_passant_file)

        self.game_state.prev_captured_piece = captured_piece
        self.game_state.en_passant_file = -1
        self.game_state.in_check = False
        self.game_state.in_double_check = False

        # handle captures
        if captured_piece_type != Piece.NONE:
            captured_square = target_square
            if is_en_passant:
                captured_square = target_square + (-8 if self.game_state.white_to_move else 8)
                self.board[captured_square] = Piece.NONE

            self.piece_count[self.game_state.opponent_color | captured_piece_type] -= 1
            self.game_state.num_pieces -= 1

            self.game_state.zobrist_key ^= self.zobrist.get_zobrist_num(captured_piece, captured_square)

        # handle king
        if moved_piece_type == Piece.KING:
            self.game_state.king_square[move_color] = target_square

            if move_color == Color.WHITE:
                self.game_state.can_white_queen_side_castle = False
                self.game_state.can_white_king_side_castle = False
            else:
                self.game_state.can_black_queen_side_castle = False
                self.game_state.can_black_king_side_castle = False
            ...
            if move_flag == Move.CASTLE_FLAG:
                ...
                self.game_state.zobrist_key ^= self.zobrist.get_zobrist_num(rook_piece, rook_start_square)
                self.game_state.zobrist_key ^= self.zobrist.get_zobrist_num(rook_piece, rook_target_square)
        ...
        if is_promotion:
            ...
            self.game_state.zobrist_key ^= self.zobrist.get_zobrist_num(moved_piece, target_square)
            self.game_state.zobrist_key ^= self.zobrist.get_zobrist_num(promotion_piece, target_square)

        # pawn has moved two forwards, mark file with en passant
        if move_flag == Move.PAWN_TWO_UP_FLAG:
            self.game_state.en_passant_file = BoardHelper.get_file(target_square)
        ...
        if prev_castling_state != new_castling_state:
            self.game_state.zobrist_key ^= self.zobrist.get_zobrist_castling_rights(prev_castling_state)
            self.game_state.zobrist_key ^= self.zobrist.get_zobrist_castling_rights(new_castling_state)

        # change side to move
        self.game_state.white_to_move = not self.game_state.white_to_move
        self.game_state.friendly_color, self.game_state.opponent_color = self.game_state.opponent_color, self.game_state.friendly_color

        self.game_state.zobrist_key ^= self.zobrist.get_zobrist_side_to_move(Color.BLACK)
    ...
    def unmake_move(self, move):
        ...
        undoing_capture = Piece.get_piece_type(self.game_state.prev_captured_piece) != Piece.NONE
        ...
        if undoing_capture:
            ...
            if undoing_en_passant:
                capture_square += -8 if self.game_state.white_to_move else 8

            self.board[capture_square] = self.game_state.prev_captured_piece
            self.piece_count[self.game_state.prev_captured_piece] += 1

        if moved_piece_type == Piece.KING:
            self.game_state.king_square[move_color] = moved_from
    ...
    def get_castling_state(self):
        ...
        castling_state |= 0b0001 if self.game_state.can_white_queen_side_castle else 0b0000
        castling_state |= 0b0010 if self.game_state.can_white_king_side_castle else 0b0000
        castling_state |= 0b0100 if self.game_state.can_black_queen_side_castle else 0b0000
        castling_state |= 0b1000 if self.game_state.can_black_king_side_castle else 0b0000
            ...
            if symbol == '/':
                ...
                if symbol.isdigit():
                    ...
                    if piece_type == Piece.KING:
                        self.game_state.king_square[piece_color] = square_index
                    file += 1

        # set player turn
        if len(fen_info) > 1 and fen_info[1] == 'b':
            self.game_state.white_to_move = False
            self.game_state.friendly_color = Color.BLACK
            self.game_state.opponent_color = Color.WHITE
        else:
            self.game_state.white_to_move = True
            self.game_state.friendly_color = Color.WHITE
            self.game_state.opponent_color = Color.BLACK

        # set castling rights
        if len(fen_info) > 2:
            fen_castling_str = fen_info[2]

            self.game_state.can_white_queen_side_castle = True if 'Q' in fen_castling_str else False
            self.game_state.can_white_king_side_castle = True if 'K' in fen_castling_str else False
            self.game_state.can_black_queen_side_castle = True if 'q' in fen_castling_str else False
            self.game_state.can_black_king_side_castle = True if 'k' in fen_castling_str else False

        # set en passant file
        if len(fen_info) > 3 and fen_info[3] != '-':
            fen_en_passant_str = fen_info[3]
            rank_to_index = {'a': 0, 'b': 1, 'c': 2, 'd': 3, 'e': 4, 'f': 5, 'g': 6, 'h': 7}
            self.game_state.en_passant_file = rank_to_index[fen_en_passant_str[0]]

        # set halfmove clock
        if len(fen_info) > 4:
            self.game_state.num_half_moves = int(fen_info[4])

        # set fullmove clock
        if len(fen_info) > 5:
            self.game_state.num_full_moves = int(fen_info[5])

        # set up initial zobrist key
        for square_index in range(len(self.board)):
            piece = self.board[square_index]
            self.game_state.zobrist_key ^= self.zobrist.get_zobrist_num(piece, square_index)

        side_to_move = Color.WHITE if self.game_state.white_to_move else Color.BLACK

        self.game_state.zobrist_key ^= self.zobrist.get_zobrist_en_passant(self.game_state.en_passant_file)
        self.game_state.zobrist_key ^= self.zobrist.get_zobrist_side_to_move(side_to_move)
        self.game_state.zobrist_key ^= self.zobrist.get_zobrist_castling_rights(self.get_castling_state())
    ...
    def __str__(self):
        ...
        for i in range(len(self.board)):
            ...
            if (i + 1) % 8 == 0:
                ...
                row = ""
        s += "   a b c d e f g h"
        s += f"\n{'White' if (self.game_state.white_to_move is True) else 'Black'} to move."
...
if __name__ == "__main__":
    board = Board()
    print(board)
    print(board.game_state.zobrist_key)
